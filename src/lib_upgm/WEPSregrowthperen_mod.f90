...
module WEPSregrowthperen_mod
  use Preprocess_mod
  use constants, only: dp, check_return, u_mgtokg
  use plant_mod
  use WEPSCrop_util_mod, only: shootnum, shoot_delay, shoot_flg, per_release
  implicit none

  type, extends(preprocess) :: WEPSregrowthperen
    contains
  ...
  end type WEPSregrowthperen

  contains

    subroutine load_state(self, processState)
      implicit none
      class(WEPSregrowthperen), intent(inout) :: self
    ...
    subroutine proc_register(self, req_input, prod_output)
      ! Variables
      implicit none
      class(WEPSregrowthperen), intent(in) :: self
            ...
            if( bctwarmdays .ge. shoot_delay ) then
              ...
              if( .not. shoot_growing ) then
                ...
                if( bcmstandleaflive .lt. pot_leaf_mass ) then
                  ! present living leaf mass less than leaf mass from storage regrowth
...
end module WEPSregrowthperen_mod
