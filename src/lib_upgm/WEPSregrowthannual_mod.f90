...
module WEPSregrowthannual_mod
  ...
  type, extends(preprocess) :: WEPSregrowthannual
    contains
  ...
  end type WEPSregrowthannual

  contains

    subroutine load_state(self, processState)
      implicit none
      class(WEPSregrowthannual), intent(inout) :: self
    ...
    subroutine proc_register(self, req_input, prod_output)
      ! Variables
      implicit none
      class(WEPSregrowthannual), intent(in) :: self
    ...
    subroutine regrowth_proc(self, plnt, env)
      ...
      do_regrow = .false.
      if( can_regrow ) then
        ! check for removal of all (most) leaf mass
                ...
                if( growing ) then
                  ...
                  if( bcmstandleaflive .lt. pot_leaf_mass ) then
                    ...
                    if ( bczloc_regrow .gt. 0.0_dp ) then
                      ...
                      ! get res value to dimension array
...
end module WEPSregrowthannual_mod
